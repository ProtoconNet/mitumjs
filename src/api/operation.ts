...
async function getBlockOperationsByHeight(
  ...
  return await axios.get(
    `${IP.from(api).toString()}/block/${Big.from(height).toString()}/operations`
  );
}
...
async function getAccountOperations(
  ...
  return await axios.get(
    `${IP.from(api).toString()}/account/${Address.from(
      address
    ).toString()}/operations`
  );
}
...
async function send(
  api: string | IP,
  operation: HintedObject | string,
  config?: { [i: string]: any }
) {
  return await axios.post(
    `${IP.from(api).toString()}/builder/send`,
...
async function delegateSend(
  ...
  const delegateAddress = "http://52.78.50.162:3398/v1/mitumt/delegate/call";
