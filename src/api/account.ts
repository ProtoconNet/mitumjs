...
async function getAccount(
  ...
  const apiPath = `${IP.from(api).toString()}/account/${Address.from(
    address
  ).toString()}`;
...
async function getAccountByPublicKey(
  api: string | IP,
  publicKey: string | Key,
  delegate?: boolean | undefined
) {
  const apiPath = `${IP.from(api).toString()}/accounts?publickey=${Key.from(
    publicKey
  ).toString()}`;
  const encodedString = encodeURIComponent(apiPath);
  return !delegate
